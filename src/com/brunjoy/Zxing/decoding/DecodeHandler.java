...
import com.brunjoy.Zxing.camera.PlanarYUVLuminanceSource;
...
final class DecodeHandler extends Handler {
    ...
    private void decode(byte[] data, int width, int height) {
        ...
        if (rawResult != null) {
            ...
            Message message = Message.obtain( activity.getHandler( ), Const.DECODE_SUCCEEDED, rawResult );
            Bundle bundle = new Bundle( );
            bundle.putParcelable( DecodeThread.BARCODE_BITMAP, source.renderCroppedGreyscaleBitmap( ) );
            message.setData( bundle );
            message.sendToTarget( );
        } else {
            Message message = Message.obtain( activity.getHandler( ), Const.DECODE_FAILED );
            message.sendToTarget( );
        }
    }
