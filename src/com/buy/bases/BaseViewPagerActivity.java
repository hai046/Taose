...
import com.buy.data.NetUtil;
import com.buy.data.NetWorkCallback;
import com.buy.stores.Settings;
import com.buy.util.BitmapUtil;
import com.buy.util.ImageDownloader;
import com.buy.util.ImageDownloader.Quality;
 ...
 */
public abstract class BaseViewPagerActivity extends FragmentActivity implements OnClickListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        // RelativeLayout include_topLayout = (RelativeLayout) findViewById( R.id.mainTitleLayout );
        // SkinManger.getInstance( this ).setBackgroundDrable( include_topLayout, "dhbj" );
    ...
    private void checkUpdata() {
        long time = Settings.getInSettings( this ).getLong( "lastCheckTime", 24 * 60 * 60 * 1000 + 1 );
        if (System.currentTimeMillis( ) - time >= 24 * 60 * 60 * 1000) {
            new CheckUpdataManager( this ).checkAndUpdata( false );
            Settings.getInSettings( this ).putLong( "lastCheckTime", System.currentTimeMillis( ) );
        }
        ...
        mNetUtil.setNetWorkCallback( new NetWorkCallback( ) {
            ...
            public void netChanger(int typeMobile, State mobile, boolean isFastNetwork) {
                ...
                case ConnectivityManager.TYPE_WIFI:
                    MyToast.showMsgLong( BaseViewPagerActivity.this, "在WIFI模式下，自动为您切换到高清模式" );
                    if (getMinWH( ) <= 0)
                        ImageDownloader.getInstance( ).setQuality( Quality.MIDDLE );
                    else {
                        Quality mQuality = ImageDownloader.getInstance( ).getNextQuality( getMinWH( ) );
                ...
                case ConnectivityManager.TYPE_MOBILE:
                    if (isFastNetwork) {
                        MyToast.showMsgLong( BaseViewPagerActivity.this, "在手机3G模式下，自动为您切换到标清模式" );
                        ...
                        MyToast.showMsgLong( BaseViewPagerActivity.this, "由于您的网速较慢，自动为您切换到极速模式" );
    ...
    private int getMinWH() {
        ...
        return (int) (Math.min( getWindowManager( ).getDefaultDisplay( ).getHeight( ), getWindowManager( ).getDefaultDisplay( ).getWidth( ) ) * 0.5);
    ...
    protected void switchView(int position) {
        ...
        mainLayout.addView( onCreateDefView( position ) );
    ...
    public void startLeftAnimation(final int from, final int to) {
        ...
        runOnUiThread( new Runnable( ) {
            ...
            public void run() {
                ...
                if (Math.min( w, h ) <= 0)
                    return;
                Rotate3dAnimation mRotate3dAnimation = new Rotate3dAnimation( from, to, w, h, 0, false );
    ...
    private void switchMenuBg(int position) {
        ...
        ImageView btn = (ImageView) linearBottomLayout.getChildAt( position ).findViewById( R.id.menu_btn );
        switch (position) {
        case 0:
        ...
        TextView tv = (TextView) linearBottomLayout.getChildAt( position ).findViewById( R.id.menu_title );
     ...
     * @return
     */
    public abstract View onCreateDefView(int index);
    ...
    public void setNaviationLeftBg(int leftImgId) {
        if (leftImgId > 0) {
            ...
            // btnLeft.setBackgroundDrawable( SkinManger.getInstance( this ).getDrawable( leftImgId ) );
        } else {
            btnLeft.setBackgroundColor( Color.TRANSPARENT );
        }
    }

    public void setNaviationBg(int leftImgId, int centerImageId, int rightImageId) {
        if (leftImgId > 0) {
            btnLeft.setBackgroundResource( leftImgId );
            // SkinManger.getInstance( this ).setBackgroundDrable( btnLeft, leftImgId );
        } else {
            btnLeft.setBackgroundColor( Color.TRANSPARENT );
        }
        ...
        if (centerImageId > 0) {
            ...
            // SkinManger.getInstance( this ).setBackgroundDrable( tvTitle, centerImageId );
        ...
        if (rightImageId > 0) {
            ...
            // SkinManger.getInstance( this ).setBackgroundDrable( btnRight, rightImageId );
        } else {
